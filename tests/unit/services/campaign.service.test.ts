...
describe('campaignService.deleteCampaign', () => {
    ...
    test('should cascade delete: donation_records → appointments → campaign', async () => {
        const mockAppointments = [{ id: 'appt-1' }, { id: 'appt-2' }];
        ...
        (supabase.from as jest.Mock).mockImplementation((table: string) => {
            if (table === 'appointments') {
                return {
                    select: jest.fn().mockReturnValue({
                        eq: jest.fn().mockResolvedValue({ data: mockAppointments, error: null }),
                    }),
                    delete: jest.fn(() => {
                        callSequence.push('delete-appointments');
                        return {
            ...
            if (table === 'donation_records') {
                return {
                    delete: jest.fn(() => {
                        callSequence.push('delete-donation_records');
                        return {
            ...
            if (table === 'campaigns') {
                return {
                    delete: jest.fn(() => {
                        callSequence.push('delete-campaigns');
                        return {
                            eq: jest.fn().mockResolvedValue({ error: null }),
                        };
                    }),
    ...
    test('should skip cascade when campaign has no appointments', async () => {
        (supabase.from as jest.Mock).mockImplementation((table: string) => {
            if (table === 'appointments') {
                return {
                    select: jest.fn().mockReturnValue({
        ...
        const result = await campaignService.deleteCampaign('campaign-empty');

        expect(result).toBe(true);
        expect(supabase.from).not.toHaveBeenCalledWith('donation_records');
    });

    test('should throw error when campaign deletion fails', async () => {
