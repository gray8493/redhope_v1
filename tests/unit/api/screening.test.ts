 ...
 * Kiểm tra API sàng lọc y tế AI (Gemini) + lưu kết quả vào screening_logs
 */
...
// Mock Supabase Admin — hỗ trợ select + insert + query chaining
const mockInsert = jest.fn().mockResolvedValue({ error: null });
const mockSingle = jest.fn();
const mockLimit = jest.fn(() => ({ single: mockSingle }));
const mockOrder = jest.fn(() => ({ limit: mockLimit }));
const mockEq3 = jest.fn(() => ({ order: mockOrder, single: mockSingle }));
const mockEq2 = jest.fn(() => ({ eq: mockEq3, order: mockOrder, single: mockSingle }));
const mockEq = jest.fn(() => ({ single: mockSingle, eq: mockEq2 }));
...
describe('POST /api/ai/screening', () => {
    ...
    describe('Eligible response', () => {
        it('should return eligible when all answers are safe', async () => {
            const aiOutput = {
                status: 'eligible',
                score: 95,
                analysis: 'Tất cả chỉ số sức khỏe đều ở mức an toàn.',
    ...
    describe('Ineligible response', () => {
        it('should return ineligible when answers have high severity', async () => {
            const aiOutput = {
                status: 'ineligible',
                score: 10,
                analysis: 'Phát hiện nhiều yếu tố nguy cơ cao.',
    ...
    describe('Edge cases', () => {
        ...
        it('should handle partial answers (only choice)', async () => {
            ...
            const aiOutput = {
                status: 'warning',
                score: 70,
                analysis: 'Thiếu thông tin tự luận.',
    ...
    /* ══════════════════════════════════════════
       ═══ Screening Logs - Lưu kết quả vào DB
       ══════════════════════════════════════════ */
    describe('Screening logs saving', () => {
        it('should save screening result to screening_logs table', async () => {
            const aiOutput = {
                status: 'eligible',
                score: 95,
                analysis: 'Sức khỏe tốt.',
            ...
            // Verify insert was called on screening_logs
            expect(mockFrom).toHaveBeenCalledWith('screening_logs');
            expect(mockInsert).toHaveBeenCalledWith(
                expect.objectContaining({
                    user_id: 'test-user',
                    ai_result: true,
                    health_details: expect.objectContaining({
                        score: 95,
                        status: 'eligible',
                        analysis: 'Sức khỏe tốt.',
                        recommendations: ['Uống đủ nước'],
        ...
        it('should save ai_result=false for ineligible status', async () => {
            const aiOutput = {
                status: 'ineligible',
                score: 10,
                analysis: 'Không đủ điều kiện.',
                recommendations: ['Tham khảo bác sĩ'],
            };
            mockAIResponse(aiOutput);
        ...
        it('should save ai_result=false for warning status', async () => {
            const aiOutput = {
                status: 'warning',
                score: 60,
                analysis: 'Cần theo dõi.',
                recommendations: ['Kiểm tra lại'],
            };
            mockAIResponse(aiOutput);
        ...
        it('should save campaignId when provided', async () => {
            const aiOutput = {
                status: 'eligible',
                score: 90,
                analysis: 'OK',
        ...
        it('should save campaign_id=null when no campaignId', async () => {
            const aiOutput = {
                status: 'eligible',
                score: 90,
                analysis: 'OK',
        ...
        it('should still return AI result even if screening_logs insert fails', async () => {
            const aiOutput = {
                status: 'eligible',
                score: 95,
                analysis: 'OK',
        ...
        it('should include answers in health_details', async () => {
            const aiOutput = {
                status: 'eligible',
                score: 85,
                analysis: 'Tốt.',
            ...
            expect(insertCall.health_details.answers).toEqual(SAFE_ANSWERS);
            expect(insertCall.health_details.analyzed_at).toBeDefined();
        });
    });
});
