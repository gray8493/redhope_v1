...
export function DashboardCarousel() {
    ...
    return (
        <div className="relative w-full h-44 rounded-2xl overflow-hidden shadow-xl group">
            <div
                className="absolute inset-0 flex transition-transform duration-500 ease-in-out h-full"
                style={{ transform: `translateX(-${currentSlide * 100}%)` }}
            >
                {SLIDES.map((slide) => (
                    <div key={slide.id} className="w-full flex-shrink-0 h-full relative p-5 md:p-6 flex items-center justify-between bg-[#0f1221] text-white">
