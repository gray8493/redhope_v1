...
export function QRCodeModal({ isOpen, onOpenChange, userData }: QRCodeModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md rounded-[2.5rem] p-0 overflow-hidden border-none bg-white dark:bg-[#1c162d]">
                <div className="bg-gradient-to-br from-red-600 to-red-900 p-8 text-white relative">
                    <div className="absolute top-0 right-0 p-8 opacity-10">
                        <Droplet className="w-24 h-24 fill-current" />
                    </div>
                    <DialogHeader className="relative z-10 text-left">
                        <DialogTitle className="text-2xl font-black italic tracking-tight">THẺ ĐỊNH DANH SỐ</DialogTitle>
                ...
                <div className="p-8 flex flex-col items-center gap-8">
                    {/* QR Code Section */}
                    <div className="bg-white p-6 rounded-3xl shadow-2xl shadow-red-500/10 border-4 border-red-50 dark:border-slate-800">
                        <QRCodeSVG
                            value={qrValue}
                            size={200}
                            level="H"
                    ...
                    <div className="w-full space-y-4">
                        <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <div className="size-12 rounded-xl bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-600">
                                <User className="w-6 h-6" />
                            </div>
                            <div className="flex-1">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Họ và tên</p>
                                <p className="text-base font-bold text-slate-900 dark:text-white uppercase">{userData.fullName}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-[10px] font-black text-red-400 uppercase tracking-widest">Nhóm máu</p>
                                <p className="text-xl font-black text-red-600">{userData.bloodGroup || "??"}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{userData.dob || "--/--/----"}</p>
                                </div>
                            </div>
                            <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{userData.city || "Việt Nam"}</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <p className="text-[10px] text-slate-400 text-center uppercase font-black tracking-[0.2em] leading-relaxed">
